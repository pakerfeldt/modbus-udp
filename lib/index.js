  ...
  var onComplete = function(err, response) {
    ...
    if (register.type == 'coil' || register.type == 'discrete') {
      ...
      for (var i in registers) {
        ...
        registerCache[registers[i]] = value;
        sendValue(registers[i], value);
        buffer.skip(length);
      }
    }
...
function findSingleRegisterConf(id) {
  for (var i in registersWithDefaultPollInterval) {
    ...
    if (index > -1) {
      ...
      conf.register.writeOnlyOnChange = register.writeOnlyOnChange;
      return conf;
    }
  }
}
...
function write(id, value) {
  ...
  if (conf.register.writeOnlyOnChange && registerCache[id] == value) {
    logger.log('info', 'Ignoring write request because value did not change');
  } else {
