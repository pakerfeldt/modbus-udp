...
function sendValue(id, value) {
  if (id === '_') {
    logger.log('debug', 'Ignoring register named _');
    return;
  }

  registerCache[id] = value;
  var msg = id + ' ' + value;
...
function pollRegisters(conf) {
  ...
  var onError = function(err) {
    logger.error('Error polling register - ' + err);
  }
...
function findSingleRegisterConf(id) {
  for (var i in registersWithDefaultPollInterval) {
    ...
    if (index > -1) {
      ...
      conf.register.writeOnAllChanges = register.writeOnAllChanges;
      return conf;
    }
  }
}
...
function writeSingleRegister(conf, value) {
  ...
  var onComplete = function(err, response) {
    if (err !== null) {
      ...
      registerCache[conf.register.id] = response.value;
...
function write(id, value) {
  ...
  if (!conf.register.writeOnAllChanges && registerCache[id] == value) {
    logger.log('debug', 'Ignoring write request because value did not change');
  } else {
